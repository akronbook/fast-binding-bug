...
const template = html<ParentComponent>`
  <div>
     <child-component @change="${(x, c) => x.onChanged(c.event)}" :status=${(x) => x.parentStatus}>
      <fast-menu>
...
export class ParentComponent extends FASTElement {
  ...
  @observable
  parentStatus: string = 'none'; // Default attribute value
  ...
  public onChanged(event: Event) {
    this.parentStatus = (event as any).detail;
  }
}